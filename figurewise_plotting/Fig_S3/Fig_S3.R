...
df <- read.csv('bush_density_1.0_no_male_movement.csv')
...
df <- filter (df, density!=0.01 & density!=0.10 ,bush_density==1)   # Bush density = 1 (structured habitat)
...
dat <- filter (df,density== 0.5) #Population density equal to 0.5
